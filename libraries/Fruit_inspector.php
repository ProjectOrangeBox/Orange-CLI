...
class Fruit_inspector {
	...
	protected function make_dummy_class_file($real_path) {
		...
		if (substr(basename($real_path),0,1) != '.') {
			...
			if (preg_match('/^(\s+)class(\s+)(?<controller>.*)(\s+)extends(\s+)/m',$file_content, $matches, PREG_OFFSET_CAPTURE, 0)) {
				if ($matches['controller'][0] != $old_class_name) {
					die('Could not locate class name "'.$old_class_name.'" in "'.$real_path.'"'.chr(10));
				}
			}

			file_put_contents($new_class_filepath,str_replace(' '.$old_class_name.' ',' '.$new_class_name.' ',$file_content));
		}

		return $new_class_filepath;
	}

	static public function clean_comments($input,$console=null) {
		$comments = '';
